...
import {
    Box, Button, Flex, FormControl, FormLabel, Heading, Text, Input, Textarea,
    useColorMode,
...
function Setting() {
    ...
    const [isChecked, setIsChecked] = useState(
        localStorage.getItem('checkboxValue') === 'true' || false
    );
    const [isCheckedNot, setIsCheckedNot] = useState(
        localStorage.getItem('checkboxValueNot') === 'true' || false
    );

    useEffect(() => {
        localStorage.setItem('checkboxValue', isChecked);
        localStorage.setItem('checkboxValueNot', isCheckedNot);
    }, [isChecked]);

    const handleCheckboxChange = () => {
        setIsChecked(!isChecked);
        // setIsCheckedNot(!isCheckedNot)
    };
    const handleCheckboxChangeNot = () => {
        // setIsChecked(!isChecked);
        setIsCheckedNot(!isCheckedNot)
    ...
    return (
        <div>
            <Box maxWidth="500px" mx="auto" p='4'>
                <Heading fontSize={'xx-large'}>Settings</Heading>
                <br />
                ...
                >
                    ...
                    <label id="theme-toggle-button">
                        <input type="checkbox" id="toggle" onClick={toggleColorMode} checked={isChecked}
